...
const FORM: InsectForm = { order: 'coleoptera', form: BEETLE };
...
describe('InsectIllustration', () => {
  it('exposes the drawing as a named image', () => {
    render(<InsectIllustration insect={FORM} seed={1} title="Spotted beetle" />);
  ...
  it('describes the beetle using the generator description', () => {
    ...
    expect(container.querySelector('desc')?.textContent).toBe(describeInsect(FORM));
  });
  ...
  it('hides itself and drops its description when decorative', () => {
    const { container } = render(
      <InsectIllustration insect={FORM} seed={1} title="Beetle" decorative />,
    );
  ...
  it('carries no inline style, so the strict CSP holds', () => {
    const { container } = render(
      <InsectIllustration insect={FORM} seed={1} title="Beetle" animate />,
    );
  ...
  it('renders the identical beetle for the same seed across mounts', () => {
    ...
    const first = render(<InsectIllustration insect={FORM} seed={7} title="Beetle" />);
    const before = geometry(first.container);

    first.unmount();

    const second = render(<InsectIllustration insect={FORM} seed={7} title="Beetle" />);

    expect(geometry(second.container)).toBe(before);
  });

  it('draws a different beetle for a different seed', () => {
    const { container: a } = render(<InsectIllustration insect={FORM} seed={1} title="A" />);
    const { container: b } = render(<InsectIllustration insect={FORM} seed={2} title="B" />);

    expect(a.querySelector('svg')?.innerHTML).not.toBe(b.querySelector('svg')?.innerHTML);
  });

  it('only marks itself animated when asked', () => {
    const { container: still } = render(<InsectIllustration insect={FORM} seed={1} title="A" />);
    const { container: moving } = render(
      <InsectIllustration insect={FORM} seed={1} title="A" animate />,
    );
