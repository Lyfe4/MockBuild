...
/**
 * The route tree, as plain data.
 *
 * It lives apart from `router.tsx` because two routers are built from it now:
 * `createBrowserRouter` in the browser, and `createStaticHandler` in
 * `entry-server.tsx` when every route is rendered to static HTML at build time.
 * A tree declared inside the call that consumes it can only be consumed once.
 *
 ...
 *
 * The static handler resolves `lazy` too, so a prerendered page is the real
 * route rather than a shell — and `main.tsx` waits for the browser router to
 * finish resolving the landing route's module before it hydrates, which is what
 * keeps the first client render identical to the file it is hydrating.
 */
 ...
 * unreachable. That matters more than it looks: the page displays the traced
 * references from `references/`, which must not ship. It also means the
 * prerenderer, which runs against a production build, cannot reach it.
 *
...
export const ROUTES: RouteObject[] = [
  {
    ...
    children: [
      ...
      ...devOnlyRoutes,
      // Last: matches anything the routes above did not. Prerendered to
      // `dist/404.html`, which is where the deploy's catch-all points.
